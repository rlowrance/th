ModelLinearSubmarketLocal <- function(InTraining, queries, data.training, formula, submarket.feature.name) {
    # return result of local linear regression for a model built on a census tract
    # return list
    ...
    #cat('start ModelLinearSubmarketLocal\n'); browser()
    verbose <- TRUE
    verbose <- FALSE
    ...
    Fit <- function(saleDate, submarket.feature.value) {
        # return list, either
        # $ok = TRUE, $fitted = lm object $num.training
        # $ok = FALSE, $feature = char, $problem = char
        #cat('start Fit\n'); print(saleDate); print(submarket.feature.value); browser()
        in.training <- InTraining(saleDate)
        data <- data.training[in.training, ]

        in.submarket <- data[[submarket.feature.name]] == submarket.feature.value
        data <- data[in.submarket,]
    ...
    FitPredict <- function(query.index) {
        ...
        submarket.feature.value <- queries[[submarket.feature.name]][[query.index]]

        fitted <- Fit(saleDate, submarket.feature.value)
        ...
        prediction <- 
            tryCatch(
                     ...
                     ,warning = function(w) {
                         cat('warning in ModelLinearSubmarketLocal:\n')
                         print(w)
                         w
                     }
                     ,error = function(e) {
                         cat('error in ModelLinearSubmarketLocal:\n')
                         print(e)
                     }
                     )
    ...
    if (verbose) {print('ModelLinearSubmarketLocal prediction'); print(predictions)}
