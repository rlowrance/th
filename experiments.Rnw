...
Figue \ref{figure:ReducedFeatureIdentification} identifies the eight models
...
models derived from PCA. Adding features to the PCA-derived models may
be helping with accuracy; if so, the PCA approach has lead to models
which may be to sparse.

The best model is the first model, which has the six most important
features found through the heuristic. The features are:
\begin{itemize}
\item living area
...
<<RidgeRegression>>=
pgm <- 'e-ridge-regression-chart'
options <- '--query-100--lambdaSet-%s'
path.a.2 <- paste0(control$working, pgm, sprintf(options, 'a'), '_2.txt')
path.a.7 <- paste0(control$working, pgm, sprintf(options, 'a'), '_7.pdf')
path.b.2 <- paste0(control$working, pgm, sprintf(options, 'b'), '_2.txt')
path.b.7 <- paste0(control$working, pgm, sprintf(options, 'b'), '_7.pdf')
path.c.2 <- paste0(control$working, pgm, sprintf(options, 'c'), '_2.txt')
path.c.7 <- paste0(control$working, pgm, sprintf(options, 'c'), '_7.pdf')
path.c.7
@

The best model identified so far is not regularized. Here we determine
if adding an $L2$ regularizer is likely to reduce the generalization
error.


\begin{figure}[h]
\caption{Ridge Regression\\First Search for Lambda}
\label{figure:RidgeA}
\centering\includegraphics[scale=.4]{\Sexpr{path.a.7}}
\end{figure}

\begin{figure}[h]
\caption{Ridge Regression\\First Search for Lambda\\Identification}
\label{figure:RidgeAId}
%\tiny
\small
%\normalsize
\verbatiminput{\Sexpr{path.a.2}}
\end{figure}

Our implementation is to use ridge regression and test various values
for $\lambda$, the weight of the $L2$ regularizer. Figure
\ref{figure:RidgeA} shows the first test set, were we see the lowest
estimated generalization error when $\lambda = 0.7$. Figure
\ref{figure:RidgeAId} has the same result in tabular form.
...
\centering\includegraphics[scale=.4]{\Sexpr{path.6}}
\end{figure}
...
<<RandomForest>>=
pgm <- 'e-random-forests-global-chart'
options <- '--hpset-a--year-2003--month-jan'
base <- paste0(control$working, pgm, options)
base
jan.2003.path.1 <- paste0(base, '_1.txt')
jan.2003.path.2 <- paste0(base, '_2.txt')
jan.2003.path.6 <- paste0(base, '_6.pdf')
@


\begin{figure}[h]
\caption{Random Forest Global Model\\January 2003 Test Data\\Identification}
\label{figure:RandomForestGlobalJan2003Identification}
%\tiny
\small
%\normalsize
\verbatiminput{\Sexpr{jan.2003.path.1}}
\end{figure}

Figure \ref{figure:RandomForestGlobalJan2003Identification} identifies
parameters for a global random forest model. The training data are for the
90 days before January 1, 2003. The test data are every transaction in
January 2003. The two hyperparameters are: 
\begin{itemize}
\item $mtry$: the number of features considered for inclusion at each
stage as a random tree is constructed. The value $mtry = 2$ means that
two features were selected at random and tested for inclusion in the
next layer of the tree. The tree being built uses the best of the
randomly-evaluated features.
\item $ntree$: the number of random tree constructed in the forest. The
literature on random forests suggests that more tree is better. X
suggests using at least one tree per training sample, and that is what
$ntree = 10803$ does as a hyperparameter.
\end{itemize}

\begin{figure}[h]
\caption{Random Forest Global Model\\January 2003 Test Data\\Cross Validation}
\label{figure:RandomForestGlobalJan2003CV}
\centering\includegraphics[scale=.8]{\Sexpr{jan.2003.path.6}}
\end{figure}

\begin{figure}[h]
\caption{Random Forest Global Model\\January 2003 Test Data\\Cross
Validation}
\label{figure:RandomForestGlobalJan2003CVExact}
%\tiny
\small
%\normalsize
\verbatiminput{\Sexpr{jan.2003.path.2}}
\end{figure}

Figure \ref{figure:RandomForestGlobalJan2003CV} shows the estimated
generalization errors from the models using the hyperparameters. The
model was trained on transactions from the 90 days before January 1,
2003 and then tested on every transaction in 2003.

Each group of 4 plot points corresponds to one setting for $mtry$ and
then incremental values for $ntree$. In all the $mtry$ cases, increasing
the number of trees ($ntree$) first decreases then estimated
generalization error and then increases it. The lowest estimated
generalization error is for $mtry = 2$ and $ntree = 1000$. Exact
values for the cross-validated loss estimates are in Figure
\ref{figure:RandomForestsGlobalJan2003CVExact}.

We used the hyperparamters found to be best for January 2003
transactions to estimate the generalized error from XXXX.
