...
model with the lowest estimated generalization error is model 7, which
has $mtry = 2$ and $ntree = 1000$. The model with one tree for each
training sample slighlty underperformed the model with 1000 trees. Exact
values for the cross-validated loss estimates are in Figure
\ref{figure:RandomForestsGlobalJan2003CVExact}


We see
that setting $mtry = 1$ seems promising, because there is perhaps a
trend in the first five data points (corresponding to $mtry = 1$ and
increasing values for $ntree$) and other choices for $mtry$ do not
appear to be have both lower estimated gneralization errors and a
consistent downward trend as more trees are added.
