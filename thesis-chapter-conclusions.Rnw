...
We found that the few random forests models were compared the local
linear model to all underperformed, but that more design work could be
applied to the random forests models.

These ideas seem potentially fruitful:

\begin{itemize}

\item Understand better why submarket models have higher errors for more
expensive properties. Is it because they trade based on features we
