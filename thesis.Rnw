...
Acknowledgements go here. Thanks to \ldots.
...
seven years instead of a single year.  The multi-year time period covers
both ups and downs in the real estate market. It also uses a much larger
population than is typical, as the data is on Los Angeles County, a
large metropolitan area. With the best linear model in hand, we compare
its performance to a non-linear model and conclude (TODO: REPLACE WITH
RESULTS FROM e-random-forests-local) that the gain in
performance from introducing non-linearity is relative small.
...
%\include{thesis-chapter-literature-review}

%\Sexpr{knit_child('thesis-chapter-data-munging.Rnw')}
\chapter{Data Munging} Dummy text.

%\Sexpr{knit_child('thesis-chapter-taxonomy.Rnw')}
\Sexpr{knit_child('thesis-chapter-using-the-2008-assessment.Rnw')}
%\Sexpr{knit_child('thesis-chapter-how-valuable-is-the-2008-assessment.Rnw')}
\chapter{How Valuables is the Year 2000 Census?}
\chapter{Selecting Linear Models Using the 2000 Census Features}
\chapter{To What Extent are Non-Linear Models An Improvement over Linear
Models?}
\chapter{Future Work}
