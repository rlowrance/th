...
% all page numbers in the center of the footer
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhead[LE]{}
\fancyhead[RO]{}
...
%\graphicspath{ {/home/roy/Dropbox/nyu-real-estate/repp-repo.git/src/local-weighted-regression-2/src/} }
...
% Do not put a chapter in the table of contents:
%  \SkipTocEntry\chapter*{Hidden Chapter Name}
...
\item Andrew Caplin, for on-the-job teaching and real partnership and
much help in navigating the system.

\item Judith Carlin, for many many proofreading sesions.

\item CoreLogic for providing the data.

\item Davi Geiger for the suggestion to through in all features in the
random forests model.

\item Leslie Greengard for continued support and encouragement.

\item John Leahy for a conversation years ago in which he conjuctured
on the efficacy of a reduced feature model. 

\item Yann LeCun for making the original offer to work on a real estate
project and for support and guidance over the years.

\item Dennis Shasha for continued interests and support over more than a
few years.

\item Marco Scoffier for explaining his trick of organization this
training data in files that each contain one column; this saved a lot of
time.

\item Damien Weldon for help in navigating through CoreLogic.
...
% these lines are just for the NYU submission process
%Predicting the Market Value of Single-Family Residential Real Estate

%Advisors: Yann LeCun, Dennis Shasha

%Author: Roy E. Lowrance

This work develops the best linear model of residential real estate
prices for 2003 through 2009 in Los Angeles County. It differs
from other studies comparing models for predicting house prices by
covering a larger geographic area than most, more houses than most, a
longer time period than most,  and the time period both before and after
the real estate price boom in the United States. In addition, it open
sources all of the software. We test designs for linear models to
determine the best form for the model as well as the training period,
features, and regularizer that produce the lowest errors.  We compare
the best of our linear models to random forests and point to directions
for further research.

%\renewcommand{\contentsname}{Table of Contents}
% Hide this chapter heading from TOC
...
%\bibliographystyle{apacite}  % missing address in bourassa-03-submarkets
