...
% draft ==> print slug in margin if line too wide
...
its performance to a non-linear model and conclude that the gain in
...
\include{thesis-chapter-data-munging}
\include{thesis-chapter-linear-models}
\include{thesis-chapter-non-linear-models}
\include{thesis-chapter-future-work}
