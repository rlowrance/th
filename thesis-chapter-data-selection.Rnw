...
value for the house. It also contains the census tract number for the
house.
...
The next few sections explore the value of the assessment and census
data.
...
questions requesting \ldots information about housing, social, and
...
censuses. The impliciation is that the earliest date we can use the
...
THE SECTION WILL BE SIMILAR TO THE ANALYSIS OF THE ASSESSMENT DATA: same
charts, different conclusions.
...
set and allows use to use transactions before and after 2008.
...
features and restrict the analysis to year beginning in 2003.
