...
The net effect is the that assessor's assessed value is below the
...
the tax roll, which we have only for 2008. Thus we cannot easily
...
set of features appearing in every transaction. The key finding is that
