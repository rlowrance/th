...
2009. A few deeds from years before 1984 are thrown in.

\item The tax roll file is for 2008. It was created in late 2007. It
...
\item The census file contains data from the decennial census in
...
conclude that the tax assessment does not carry much predictive value.
...
are fitted and used for prediction.
...
To estimate the generalization error, we use 10-fold cross validation,
...
randomly to one of 10 folds and then trains 10 sets of models. We use
the standard 10-fold cross validation approach: for each fold, we have a
training set with 90 percent of the data and testing set with 10 percent
of the data.
...
using the future in the training process, we first discard all data in
the training set that occurs on or after the date of the query
transaction. A model is then trained on the remaining data in the
training set. The trained model is then used to predict the query
transaction's price and the results are recorded.

There are many local models to be fit. Just before the models are
fitted, the feature set is analyzed to detect and eliminate any feature
that will not meet the requirements of the underlying model. For
example, usually the model requires that no feature has the same value
for every transaction.  This is an effectiveness tradeoff: we keep the
transaction in the fold's test set using a fewer features than are
specified in the model rather than discarding the test transaction. In a
fold, usually very test transactions have discarded features.

One optimization speeds up the fitting process. All models with the same
...
%2007 starting in July, when the fiscal year begins. Because property
%values were changing rapidly around the time that the tax bills were
%sent out, properties transacting before July in 2007 were assessed at
%some value other than market value.  The figure reflects recording
%dates, which tend to lag sales dates by a few months.  Thus the
%properties reported as transacting in November and December in many
%cases would have transacted a few months early, say ending in roughly
%October. The last day for publishing tax assessments is October 31.

Now we have the background to hypothesize an explanation for the many
zero-error points in the second half of 2007 in Figure
\ref{figure:AssessmentExactlyEqual}. The assessor set the assessment
for 2008 equal to the transaction prices for properties transacted in
2007 starting in July, when the fiscal year begins. Assessed values for
...
\item From the U.S. Census Bureau decennial census in year 2000.
  ...
  are owned by the occupants; median household income; whether the census
...
%One might choose to predict the
...
%At least three ideas might lead one to
...
%is to reflect a hypothetical data generating process. If a unit change in the
%feature is thought to generate a fixed change in the response, no
%transformation is appropriate. If a unit change in the feature is
%thought to generate a percent change in the response, transforming the
%feature into the log domain is appropriate. Computationally, the size
%variables are partitioned into those that may be zero and those that are
%always positive. Those that may be zero are transformed into the log
%domain by adding one and taking the log. The positive variables are
...
training period. Each cell after column one contains the
median of the RMSE values from 10-fold cross validation. Here RMSE means
the square root of the median squared errors. Thus each cell
...
leads to higher estimated errors.
...
census blocks are further aggregated into several hierarchies
...
we use are from the 2000 census for Los Angeles County and are at the
census tract level.
...
Unfortunately, the census tract numbers we use for properties are
contained in the tax roll, which we have only for 2008. Thus we cannot
...
exactly, we have assumed that transactions on or after January 1, 2003
...
By Response Variable, Predictors, and Training Period\\
For A One Percent Sample of Sales in 2003 and Later
}
...
root median squared errors from the folds. The key finding is that
using the census-derived features always leads to better performance.
...
features and restrict the analysis to year 2003 and later.
