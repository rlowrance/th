...
resulting in a non-linear model. In our experiments, carefully designed
local linear models outperformed off-the-shelf random forests.
...
describes a random forests model that performs worse than the best of
the local linear models.  Chapter 6 presents overall conclusions that
lead to future work I'd like to do.
