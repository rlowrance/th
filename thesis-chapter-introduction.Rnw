...
increase from the minumum.

%\item One would hope that a few features of houses would contain most of
%the information that is driving prices. If so, models could be simpler.
%We found that selecting the \Sexpr{count.parsimony.features} most
%important features would reduce the median error significantly, from
%\$\Sexpr{Commas(error.1)} using only the most important feature to
%\$\Sexpr{Commas(error.6)}, a reduction of \Sexpr{Percent(p.1.to.6)}
%percent. The minimum error of \$\Sexpr{Commas(error.15)} occurs when
%using the \Sexpr{count.min.features} most important features, a slight
%reduction of \Sexpr{Percent(p.6.to.15)} percent from using only the
%\Sexpr{count.parsimony.features} most important features.
%
%
%OLD
%
...
%important features has an estimated error that is 20 percent higher.
%Using more features means more opportunities for overfitting, and we
%find that regularization improved predictive accuracy.
...
local linear model, a separate model is fit to every query transaction,
resulting in a non-linear model. We demonstrate the local linear models
are likely to outperform random forests when predicting house prices.
