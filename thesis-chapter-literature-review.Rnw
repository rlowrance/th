...
Several distractors keep researchers away from the field.  The
...
opportunity to buy data for Dade County, Florida for a dollar a
record. Sometimes the data is priced and sometimes it is free. It seems
that the Louisville, Kentucky tax assessor has made Louisville data freely
...
have a partnership which obtains the datasets from the tax authorities
and then cleans and augments them and finally places them into a
canonical form, because not all counties produce the same datasets for
...
components. The extracted clusters were define to be the submarkets;
...
was measured as the fraction of estimated prices that are within 10 and
...
In addition the features from model 1 were used as well as interactions
with the indicator variables. Though nine multiple listing systems were
used, only three indicator variables for them were included. The choice
of these variables reflects the expert opinions of real estate agents,
who grouped some of the multiple listing services.
...
Virginia over 1967 through 1991.  The time period was long but covers
...
the product of these features. The trend surface is very similar to the
$x,y$ approach from \cite{fik-03-spatial}, which was reviewed in the
previous section.  A variant in the model 1 family included indicator
variables for census tracts, building what is now called submarket
models. Model 1 variants also included indicator variables for the
years.
...
terms was modeled explicitly. The model was (we follow the text, page
176) $Y = \beta X + \mu$, where $\mu$ is drawn from $N(0, \sigma^2 K)$,
a change from the standard OLS model in which $\mu$ is drawn from $N(0,
\sigma^2 I)$, where $I$ is the identity matrix. If $K$ were estimated by
$\hat K$, then the estimate for $\beta$ would be $\hat \beta = (Y'{\hat
K}^{-1} X) ^ {-1} X' {\hat K} ^ {-1} Y$.
...
$K$. Kriging is based on the assumption that $K$,  the correlation
matrix for the error terms, is a function solely of the distance between
each observation $i$ and $j$. The model builder Robin Dubin assumed this
form for $K$: $K_{ij} = b_1 \exp(-d_{ij}/ b_2)$ when $ i \neq j$ and
$K_{ij} = 1$ when $i = j$. Here $d_{ij}$ was the physical distance
separating house $i$ and $j$.  The parameters $K_{ij}$, $b_1$, $b_2$,
and $\sigma$ were estimated through maximum likelihood.
...
Property Valuation Administrator for Jefferson Country, which then
...
geostatistical model suggests that improvements are to be found. One is
interested in modeling the errors and an approach other than modeling the
covariance of the errors may be productive.
...
dataset is relatively small (13,000 transactions).
...
cannot be extended to also test other techniques indepedent of the
original study team.
...
hidden layer is a radial Gaussian activation function.
...
never going to be able ``to stand on the shoulders of giants'' (the
metaphor is attributed to Bernard of Chartes by John of Salisbury in a
book published in 1159 \cite[p.~167]{salisbury-1159}). Though we may be
able to identify the giants (this study is one of them), we can't use
...
up the data sets. All the modelling code is designed to be adapted to
...
\item Most prior studies are for relatively small datasets. Our dataset
is for all of Los Angeles County, the most populated country in the
...
\item We are paranoid about using future information in fitting models.
We avoid the future through careful design of the datasets and by
fitting a model to each query transaction in a way that the all data on
or after the date of the query transaction are invisible to the fitting
process. 
...
\item We focus on linear models, which are not the best in the
literature as judged by their accuracy: they are roughly 5 percent less
accurate than the best models. However, they are widely popular and many
issues concerning their design are studied here for what we believe to be
the first time.
