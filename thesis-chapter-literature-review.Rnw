...
\item There is some art in determining which interactions to include and
...
terms is modeled explicitly. This approach is called ``kriging.'' One
...
simpler easier-to-compute and easier-to-understand approaches. One such
...
\item A geostatistical model, an
...
set is for all of Los Angeles County, the most populated country in the
...
We avoid the future through careful design of the data sets and by
fitting a model to each query transaction in a way that the all data or
are after the date of the query transaction is invisible to the fitting
process. 
\end{itemize}
...
issues concerned their design are studied here for what we believe to be
the first time.
