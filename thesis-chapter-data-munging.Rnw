...
We start with real estate data for Los Angeles County: the tax roll for
...
bills. In Los Angeles County, the initial real estate bills are send
...
with other data, and licenses them. There is one record (CoreLogic type
...
so forth. The LUSEI field is used to identify whether the parcel is for
...
of rooms, number of bedrooms, number of bathrooms, and whether it has a swimming
pool. Many of the values are missing. Later we describe how we define a
...
licenses them. There is one record (CoreLogic type 1080) for every deed.
...
The census file for the year 2000 decennial census. It contains records
...
\Sexpr{Commas(About(deeds$info$num.all.deeds))}
deeds of which  
\Sexpr{Commas(About(deeds$info$num.is.arms.length))} 
...
\Sexpr{Commas(About(deeds$info$num.is.grant.deed))} 
grant deeds in the deeds files.

We are interested in deeds that are both arms-length and grant deeds.
There are 
\Sexpr{Commas(About(deeds$info$num.is.arms.length.and.grant.deed))} 
such deeds.
...
\Sexpr{CommasAbout(parcels$info$nrow.all)} 
tax roll records of which 
\Sexpr{CommasAbout(parcels$info$nrow.sfr)}
...
\begin{itemize}
  ...
  \item The file of all single-family-residence parcels containing 
    \Sexpr{CommasAbout(transactions$info$nrow.parcels.sfr)}
  ...
  \item The file containing 
    ...
    \Sexpr{CommasAbout(transactions$info$nrow.census.deeds.parcels)} records
    results.
  \item The file containing
...
\begin{itemize}
  ...
  \item Assessed value. Transaction arising from parcels with an
    ...
    There was \Sexpr{Remaining(subset$info$assessed.value$num.not.too.large)} such.
...
%    such.
  ...
  \item Effective year built. Transactions arising from properties
    ...
    is built. There were \Sexpr{Remaining(subset$info$effective.year.built)} such.
  ...
  \item One building. Transactions arising from parcels with more
    than one building were rejected because we have only the description
    for one building. There were \Sexpr{Remaining(subset$info$one.building)}
    such.
  ...
  \item Sale amount. Some deeds report extremely large prices.
    ...
    \$\Sexpr{Commas(as.integer(subset$control$max.sale.amount))}, 
    so observations with higher prices were rejected.  There were
  ...
  \item Transaction type code. We rejected transactions for parcels
    that were neither new construction nor resales. Other
  ...
  \item Year built. Transactions arising from properties with a
    missing year built were discarded. There were
...
\begin{itemize}
  \item Land square footage. The square footage of the land. The
    largest land square footage value in the transactions file is 
