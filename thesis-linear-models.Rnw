...
%parameters for a global random forest model. The training data are the
...
%next layer of the tree. The best one was included.
%\item $ntree$: the number of random tree constructed in the forest. The
%literature on random forests suggests that more tree is better.
%\end{itemize}
%
%\begin{figure}[h]
...
%generalization errors from the models using the hyperparameters. We see
