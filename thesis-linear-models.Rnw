...
Control <- function() {
    ...
    AppendAdjustTrainingPeriod <- function(lst) {
    ...
    control10 <- AppendAdjustTrainingPeriod(control9)
}
...
see that the best training period is often 30 days or 180 days. Thi sis
consistent with supposing that a short training period is appropriate
when prices are volating and a long training period is appropriate when
prices are stable. Thus there may be merit to the idea of tuning the
training period to the volatility of prices at the time of the query,
and the proposed method simply fails to implement that idea.
...
Future work could explore two alternatives to fixing the training period
once across the 25 years of data, as was examined in these experiments.
One alternative is to include dummy variables for the time period. The
dummy captures average price levels during the time period.  To
forecast, one needs to forecast the value of the dummy variable forward
one day. A simple linear estimate for the time-period dummy may be
sufficient.

Another alternative is to adapt the training period to the volatility in
the market: when prices are more volatile, train on short periods, when
prices are more stable, train on longer periods. This idea could be
implemented by using cross-validation to select the best training period
just before running the model for a given query date. One business
setting in which this approach would apply is the very common one of
predicting prices for all properties one day into the future.
