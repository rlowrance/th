...
So far in this work, the features used as predictors have been based on
those selected by Sumit Chopra for his thesis work \cite{chopra-09}. He
selected features that were present in many observations. However
additional features are possible and the features used so far are not
necessarily the best ones.  Indeed, Kuhn-13 chapter 19 claims that
adding non-informative predictors to a linear model can degrade its
predictive accuracy (TODO: cite the original paper, if possible).

This section explores whether some predictors in the Chopra-derived set
should be dropped and whether certain additional predictors should be
added.

We use two approaches to name features that may be important in
prediction accuracy. In the first approach, we develop a
heuristic to approximate the features that are best. We start with all
the features Chopra considered and then add about 15 additional features
to build a comprehensive set of candidate features. In the second
approach, we use PCA to identify features. Each of these approaches
suggests one or more sets of features that may be important. We then
take all the suggested feature sets and use cross validation to estimate
the generalized error from each.

\subsection{Possibly dropping some of Chopra's features}

Our predictor feature set has 11 elements. A straight-forward way to
determine whether a subset of these feature provides more accurate
predictions would be to form every subset of the 11 features and use
cross validation to estimate the generalization error for each of the
subset of features. This approach does not scale well, as the number of
possible models to test grows factorially with the number of features.

So we developed a heuristic that proceeds in two stages. In stage one,
we estimate the value of adding each feature in order to a base
model which initially has no features. Thus we determine the best
feature to add initially, and given, that this feature is in the model,
the next feature to add second, and so forth. This greedy procedure has
time requirements of order $O(n^2)$ in $n$, the number of possible
predictors, assuming that the time to determine which feature to add
next is linear. It provides a rank-ordering of the value of the
features. In stage two, we then estimate the generalization error from
using just the first of the rank-ordered features, then the first and
second, then the first and second and third, and so forth. The second
stage takes time $O(n)$. This heuristic approximates the factorial-timed
problem with a $O(n^2)$ time problem.

The first stage is implemented by running the elastic net procedure from
Zou and Hastie (\cite{zou-05}).  This procedure is claimed to implement
both the lasso and ridge regression efficiently in the time required for
ordinary least squares (page 309). For our purposes, we don't need the
ridge regression results, just the rank ordering of feature importance
in accuracy.

The second stage is implemented by using the rank-ordered features to
derive models that are studied in a 10-fold cross validation procedure.

<<Dropping>>=
pgm <- 'e-features-lcv-chart'
options <- '--predictors-chopra--query_fraction-0_010000'
base <- paste0(
control$working
,pgm
,options
...
path.2 <- paste0(base, '.2.txt')
path.3 <- paste0(base, '_3.pdf')
path.4 <- paste0(base, '.4.pdf')
path.5 <- paste0(base, '.5.pdf')

path.1
substr(path.1, 50, 200)

path.3
substr(path.3, 50, 200)

control$path.in.e.median.price.by.month.pdf
substr(control$path.in.e.median.price.by.month.pdf, 50, 200)
@
...
%
%\begin{figure}[h]
%\caption{Dropping Features 1 Percent Sample Feature Names}
%\label{figure:Dropping01Names}
%%\tiny
%%\small
%\normalsize
...
%
%\begin{figure}[h]
%\caption{Dropping Features 1 Percent Sample Cross Validation Errors}
%\label{figure:Dropping01Chart}
%\centering\includegraphics[scale=.4]{\Sexpr{path.3}}
%\end{figure}
%
Figure \ref{figure:Dropping01Names} contains the names of the features
sorted in order of importance as found by the elastic net algorithm.
Figure \ref{figure:Dropping01Chart} is a box chart depicting the median
errors found in each validation fold.

We see that WRITE ME.


depicts the results of applying the
heuristic to a 0.1 percent random sample of the test transactions in
every fold using 10-fold cross validation. The most important feature is
the living area and perhaps surprisingly the least important is the
number of bathrooms. One possible explanation for the low importance of
the number of bathrooms is that it might be highly correlated with the
number of bedrooms, which is ranked as more important. The second most
important feature is the median household income in the census tract.
This feature is measured in the year 2000, so it does have future
information for many of the test samples. However, it is none-the-less a
proxy for the quality of the neighborhood and we are not surprised to see
that it is a very important feature. Features ranked as third and
fourth in importance are also features of the census tract. Features
ranked 2 through 4 are all location features and real estate agents
claim that location is the most important feature when pricing a house.
This experiment suggests that location is in fact the second most
important feature, behind living area, and provides a way to measure
location quality.

Turning to the second part of the figure, we see that the estimated
generalization errors as measured by RMedianSE is lowest when the first
9 features are used and increases when features ranked 10 and 11 are
added. Dropping the new construction and bathrooms features is suggested
as a way to improve prediction accuracy.

Moreover, the RMedianSE increases after the three location features are
added and then decreases until the 9th feature has been added. Because
the RMedianSE values for the 4th and 9th models are very close,
dropping all but the first 4 features is suggested. If this were done,
we would have a very reduced model: house size plus location features.
This model has many fewer features than typical models in the house
price literature.

%
%\begin{figure}[h]
%\caption{Dropping Features 1 Percent Sample}
%\label{figure:Dropping01}
%%\tiny
%\small
%%\normalsize
%\verbatiminput{\Sexpr{path.01}}
%\end{figure}
%
%Figure \ref{figure:Dropping01} contains the same analysis but for a 1
%percent sample of test transactions in the folds. The rank-ordering of
%the features is the same as for the 0.1 percent sample because of the
%way the heuristic is implemented. 
%
%We see that as for the 0.1 percent sample, the living area and three
%location features provide improved estimated RMedianSE values, but now
%using the number of parking spaces and whether the property has a pool
%also provide improved estimated accuracy. The minimum RMedianSE is again
%for the 9-feature model and the RMedianSE for it is close the the
%RMedianSE for the 6 factor model. One would consider using 6 factors
%instead of 9. Adding the last two features (new construction and number
%of bathrooms) degrades accuracy.
%
%Now we can update our re-write of what real estate agents say: In Los
%Angeles, the most important features for house prices are the size of
%the house, its location, the number of parking spaces, and whether it
%has a pool. To me, this seems consistent with my impression of Los
%Angeles: its crowded so that on-street parking is hard to find; its hot
%so that swimming pools are valuable.
%
%The findings from these two experiments show that feature selection is
%in fact an issue when designing linear models for house price
%prediction. The findings do not contradict the claim that adding
%non-information features can reduce prediction accuracy for linear
%models (but we have not shown the features to be non-informative).
%
%
\subsection{Possibly adding some feature to Chopra's feature set}
%
%We now consider adding features beyond the 11 features that we have been
%considering so far. One set of features to consider are those that occur
%in every one of the transactions in our training data. Including these
%additional features means that one does not have to develop an approach
%that handles missing values.
%
%<<AddingFeatures>>=
%path.adding <- control$path.in.e.features.lcv.always.010000
%path.adding
%substr(path.adding, 50, 200)
%@
%
%\begin{figure}[h]
%\caption{Adding Features 1 Percent Sample}
%\label{figure:Adding01}
%%\tiny
%\small
%%\normalsize
%\verbatiminput{\Sexpr{path.adding}}
%\end{figure}
%
%Figure \ref{figure:Adding01} shows the results of applying our heuristic
%to the 25 features that are always present. As shown in the first part
%of the figure, the most important feature is the living area of the
%house. Second most important is the income of the census tract.
%
%As shown in the second part of the figure, the estimated
%generatalization error declines are the first 8 features are included in
%the model, then increases and then declines until 16 features are
%included in the model, and then the pattern repeats. The lowest
%estimated generalization error is for the model with 23 features.
%
%We make these observations
%\begin{itemize}
%\item The 16-feature model only slightly outperforms the 8 feature
%models. 
%\item The 21-feature model only slightly outperforms the 16-feature
%model.
%\item The 23-feature model only slightly outperforms the 21-feature
%model.
%\end{itemize}
%
%Any of these models could be good choices. If one prefers parsimonious
%models, the improvements in estimated accuracy after 16 features are
%included are very small.
%
%If the 8-feature model were used, the most important features of the
%house would be its living area, the number of fireplaces, the year it
%was last remodelled (effective year built), and the total number of
%rooms. The most important features of the neighborhood would be its
%median household income, the average commute time, fraction owner
%occupied, and whether the zip code has any parcels that are classified
%as industry. Both sets of features are consistent with my intuition.
%
%Looking at the first 16 features, the features of the house can be
%grouped as follows:
%\begin{itemize}
%\item Features related to the size of the house: living area, total
%rooms, parking spaces, land square footage, stories number.
%\item Features related to the amenities of the house: fireplace number,
%effective year built, factor has pool.
%\end{itemize}
%
%Again looking at the first 16 features, the features of the neighborhood
%can be grouped as follows:
%\begin{itemize}
%\item Features related to social class of the neighbors: median
%household income, fraction owner occupied. More expansive features of
%the house are additiionally indicative of the social class of the
%neighbors.
%\item Features related to amenities of the location: average commute
%time, census tract has industry, zip5 has pool, census tract has retail,
%and zip5 has school.
%\end{itemize}
%
%Real estate agents claim that the most important three features of a
%house are location, location, and location. This work suggests that the
%house itself is important as well.
%
%
...
The LCV analysis suggests testing these models: WRITE ME.
...
Models: 8 features, 16 features, PCA features (median household income,
land square footage, basement square feet) all 25 features, all 11
chopra feature, all 11 chopra features
