...
use is smooth, but that our granulatity in estimating it is too large:
...
experimenta. Thus there may be merit to the idea of tuning the training
...
predictors. It provides a rank-ordering of the value of the features. In
stage two, we then estimate the generalization error from using just the
first of the rank-ordered features, then the first and second, then the
first and second and third, and so forth. The second stage takes time
$O(n)$. This heuristic approximates the factorial-timed problem with a
$O(n^2)$ time problem.
