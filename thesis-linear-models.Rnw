...
Zou and Hastie (\cite{zou-05}).
This procedure is claimed to implement
both the lasso and ridge regression efficiently in the time required for
ordinary least squares (page 309). For our purposes, we
don't need the ridge regression results, just the rank ordering of
feature importance in accuracy.
...
<<Dropping>>=
path.001 <- control$path.in.e.penalized.regression..query.fraction.001
path.001
path.01 <- control$path.in.e.penalized.regression..query.fraction.01
path.01
@
...
\begin{figure}[h]
\caption{Dropping Features 0.1 Percent Sample}
\label{figure:Dropping001}
%\tiny
\small
%\normalsize
\verbatiminput{\Sexpr{path.001}}
\end{figure}

Figure \ref{figure:Dropping001} depicts the results of applying the
