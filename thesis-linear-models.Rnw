...
scenario, the training period is before and after the date of the query
transactions. 
...
are nearer the test transactions. Ignoring the use cases with the
assessments, we see that the median error decreases by over 20,000
dollars between the assessor and avm scenarios.  The avm scenario, by
selecting training data based on sales date and not recording dates as
for the assessor scenario, has more contemporary data, leading to
increased accuracy. Considering only the use cases with the assessment,
we see that median error always decreases as the training data become
more contemporary with the testing data. The lowest error is for the
mortgage scenario, which benefits from seeing the future prices of
transactions.

TODO: get actual median price for the test period
...
training period. As for the 30-day training period, the estimated
generalization error increases when the assessment is used as a feature.
In this use case the prices have fallen even further from the mid point
of the training period and the assessment's value is even less.
...
training period. Now the mid-point of the training period is even more
distant that in the other use cases and the value of the assessment is
negative.
...
<<CompareEAvmVariants>>=
print(control)
LoadResult <- function(path) {
  print(path)
  loaded <- load(path)
  str(loaded)
  print(all.result$experiment.name)
  all.result
}
result.30 <-
  LoadResult(control$path.in.e.avm.variants.training.30.RData)
result.60 <-
  LoadResult(control$path.in.e.avm.variants.training.60.RData)
result.90 <-
  LoadResult(control$path.in.e.avm.variants.training.90.RData)

MedianError <- function(scenario, ndays) {
  result <- if (ndays == 30) result.30 
            else (if (ndays == 60) result.60
                  else result.90
                  )
  for (rowIndex in 1:nrow(result)) {
    if (result$experiment.name[[rowIndex]] == scenario) 
      return (result$median.RMedianSE[[rowIndex]])
      }
    stop('bad scenario')
}
MedianError('mortgage with assessment', 60)  # test
...
Looking across the assessor and avm use cases, both of which train on
historic data, we see that using a longer training period always
degrades accuracy in these experiments.

Looking across the mortgage use cases, which trains on a 50-50 mix of
historic and future data, we see that in some use cases, using a longer
training period can improve accuracy. Accuracy decreased when the
training period was increased from 30 to 60 days and decreased when the
training period increased from 60 to 90 days. Accuracy for the mortgage
model was highest when the training period was longest in these
experiments.
