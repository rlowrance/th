...
60-day training period leads to the lowested errors on transactions the
...
day before the query transaction, then the test period for detemining
the best training period is extended day by day into the past until we
have found some test transations.
...
use is smooth, but that are granulatity in estimating it is too large:
...
Returning to the intepretation of the results for the two figures, we
...
short training period is appropriate when prices are volating and a long
training period is appropriate when prices are stable. The modal nature
of the exteme values also suggests assessing the accuracy of models with
both shorter and longer training periods. Thus there may be merit to the
idea of tuning the training period to the volatility of prices at the
time of the query, and the proposed method simply fails to implement
that idea.

\subsection{Ideas for Tuning the Training Period}
...
shorter training periods for times of higher volatity and longer training
...
additional features are possible and the features used are not
...
the next feature to add second, and so forth. This greedy procedure is
of order $O(n^2)$ in $n$, the number of possible predictors. It provides
a rank-ordering of the value of the features. In stage two, we then
estimate the generalization error from using just the first of the
rank-ordered features, then the first and second, then the first and
second and third, and so forth. The second stage takes time $O(n)$. This
heuristic approximates the factorial-timed problem with a $O(n^2)$ time
problem.

The first stage is implemented by running the elastic net procedure from
Hastie (TODO: provide a reference). This procedure is claimed to implement
both the lasso and ridge regression efficiently. For our purposes, we
...
proxy for the quality of the neighorhood and we are not surprised to see
...
as a way to improve predicition accuracy.

Moreover, the RMedian SE increases after the three location features are
added and then decreases until the 9th feature has been added. Because
the RMedianSE values for the 4th and 9th feature are very close,
...
using the number of parkig spaces and whether the property has a pool
