...
options <- '--predictors-chopra--query-100'
base <- paste0(
control$working
,pgm
,options
...
\begin{figure}[h]
\caption{Dropping Features 1 Percent Sample Chopra Features Rank Ordered}
\label{figure:Dropping100ChopraNames}
%\tiny
%\small
\normalsize
...
Figure \ref{figure:Dropping100ChopraNames} contains the names of the features
sorted in order of importance as found by the elastic net algorithm.
We see that the most important feature is the living area. Perhaps
surprisingly, of low importance is the number of bedrooms and bathrooms,
both of which are highly advertised in commercial real estate. Both of
these features may be highly correlated with living area and hence have
low ranking.

The second most important feature is the median household
income in the census tract.  This feature is measured in the year 2000,
so it does have future information for many of the test samples.
However, it is none-the-less a proxy for the quality of the neighborhood
and we are not surprised to see that it is a very important feature.
Features ranked as third and fourth in importance are also features of
the census tract. Features ranked 2 through 4 are all location features
and real estate agents claim that location is the most important feature
when pricing a house.  

This experiment suggests that location is in fact
the second most important feature, behind living area, and provides a
way to measure location quality.

\begin{figure}[h]
\caption{Dropping Features 1 Percent Sample Chopra Features Models Compared}
\label{figure:Dropping100ChopraChart}
\centering\includegraphics[scale=.8]{\Sexpr{path.3}}
\end{figure}

\begin{figure}[h]
\caption{Dropping Features 1 Percent Sample Chopra Features Models
Compared Median Standard Error}
\label{figure:Dropping100ChopraChart}
...
\begin{figure}[h]
\caption{Dropping Features 1 Percent Sample Chopra Features Models
Compared Mean Standard Errors}
\label{figure:Dropping100ChopraChart}
\centering\includegraphics[scale=.8]{\Sexpr{path.5}}
\end{figure}

Figure \ref{figure:Dropping100ChopraChart} is a box chart depicting the median
errors found in each validation fold. The first model (tick 1) is the
model containing the most important variable, namely the living area.
The second model contains the first two most-imporant features: living
area and median household income. The eleventh model contains all 11
features in the Chopra feature set. The value depicted is the median
across the 10 folds of the rootMedianSquaredErrors from each fold.

We see that model 9 (with the first 9 features) has the lowest estimated
generalization error. It is perhaps the best model to consider. Another
choice is model 4 (with the first 4 features), which has a median
rootMedianSquaredError that drops a lot compared to model 3. It is a
surprise to me that incremental predictive power is assigned to the
fourth variable (fraction of houses that are owner occupied).

\subsection{Possibly adding some feature to Chopra's feature set}

<<DroppingAlways>>=
pgm <- 'e-features-lcv-chart'
options <- '--predictors-always--query-100'
base <- paste0(
control$working
,pgm
,options
...
\begin{figure}[h]
\caption{Dropping Features 1 Percent Sample More Features Rank Ordered}
\label{figure:Dropping100MoreNames}
%\tiny
%\small
\normalsize
\verbatiminput{\Sexpr{path.1}}
...
Figure \ref{figure:Dropping100MoreNames} shows the rank-ordering of
features in an expanded feature set. We have included all the features
in our data that occur in all transactions.

We again see that as for the Chopra features, the living area and median
household income are the most important features. The other location
features (average commute time and fraction owner occupied) are also
highly important. The importance of the number of fireplaces was a
surprise to me; perhaps having a fireplace is a sign of a high-quality
house.

\begin{figure}[h]
\caption{Dropping Features 1 Percent Sample More Features Models Compared}
\label{figure:Dropping100MoreChart}
\centering\includegraphics[scale=.8]{\Sexpr{path.3}}
\end{figure}

\begin{figure}[h]
\caption{Dropping Features 1 Percent Sample More Features Models
Compared Including Standard Errors Median}
\label{figure:Dropping100MoreChart}
\centering\includegraphics[scale=.8]{\Sexpr{path.4}}
\end{figure}
...
\begin{figure}[h]
\caption{Dropping Features 1 Percent Sample More Features Models
Compared Including Standard Errors Mean}
\label{figure:Dropping100MoreChart}
\centering\includegraphics[scale=.8]{\Sexpr{path.5}}
\end{figure}

Figure \ref{figure:Dropping100MoreChart} is a box chart for all the
model. The first tick has the most important feature (living area), the
last tick has all 25 features. Focusing on the median values across the
folds, we see that the lowest value is for 23 features, and that model
may be a good choice. Feature not included in the 23rd model are whether
the zip code has retail parcels (but included is whether the census
tract has retail parcels) and the size of the garage (but included is
...
The LCV analysis suggests testing these models: 
\begin{itemize}
\item The model with first 9 of Chopra's features: it has the lowest estimated
generalization error.
\item The model with the first 4 of Chopra's features: it is sparse in
features and has a significantly-reduced estimated generalization error
compared to a model with the first 3 of Chopra' features.
\item The model with the first 23 of the expanded features: it has the
lowest estimated generalization error.
\item The model with the first 5 of the expanded features: it has a much
lower estimated generalization error than the model with the first 4
expanded features.
\end{itemize}
...
Thus we ran a cross validation study on the seven suggested models. We
found WRITE ME
