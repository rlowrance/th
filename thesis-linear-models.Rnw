...
test data in the cross validaition process.
...
guage the best model: median RMedianSE or mean fraction within 10
percent.
...
days. This experiment used a 0.1 percent random sample of the test data
in each cross validation fold.

BLAH FOR 1 PERCENT SAMPLE.

\subsection{Ideas for Tuning the Training Period}

Future work could explore two alternatives to fixing the training
period. One alternative is to include dummy variables for the time
period. The dummy captures average price levels during the time period.
To forecast, one needs to forecast the value of the dummy variable
forward one day. A simple linear estimate may be sufficient.
...
just before running the model for a given query date. This would be the
use case in a common scenario in which a model was extended to price
transactions that occured in the next day.
