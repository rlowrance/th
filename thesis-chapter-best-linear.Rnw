...
path.14.rf <- paste0(working, 'e-cv-chart_chart14.txt')
@ 
...
\cite[footnote on p.~642]{fik-03-spatial} to use that metric to judge
...
that are close when accurate and do not minimize the average error.
...
we consider the estimated generalization error from a variety of model.
...
the root mean squared errors from the folds (\code{meanRMSE}), the
median of the root median squared errors from the folds
...
in the training period for each query.  An asterisk preceeds the value
...
periods. The biggest diference in training periods is for the level-log
form.
...
metric to select models is not very determining: one should pick the
metric based on some other criteria.

Going forward, we focus on the \code{medRMSE} metric, judging that it
more intutitively sizes average errors than does \code{meanRMSE}
(because of the right-skew in the prices) and
reflecting our predisposition to minimize average errors rather than the
...
model trained on 60 days of data in log-level form,  we then show that
...
predicted using features are in the log domain, level-level in which
price is predicted using features in the natural units, and level-log in
which price is predicted using features in the log domain.
...
minimize errors on more typical less expensive houses, not to minimize
the price-weighted error, which could be regarded as overly-influenced
by a few very large houses.
...
\cite{zou-12} produces a rank-ordering of the features in terms of their
importance. In our use of this package, we set the L2 regularizer to
zero and see just the rank-ordering of features under the L1
regularizer.
...
possible feature set is the first 20 features (through census tract has
...
error than for the 20-feature choice, the 95 percent confidence interval
is smaller.
...
features. Our models are local, so that we need a house age for every
...
effective year built, which is the last year of signficant updates to
the house.
...
percent confidence intervals graphically. One might choose to use only
the first 3 features based on this graphic.
...
one with the first 20 features.
...
intervals are not signaling warnings.
...
overfitting in the un-regularized model. The reason for that is possibly
...
The apparently pleasing results in Figure \ref{figure:Submarkets13} hide
...
separate cities. The median errors for each city vary widely. Figure
\ref{figure:Examples13} shows the the medians for various cities. We see
that the most accurately-estimated houses are those in Lancaster, where
the estimated generalization error is about \$15,000. What is common
about the cities with lowest estimated errors is not obvious to me. Turning
to the cities with the ten highest estimated errors, we see cities that
expensive: Santa Monica, Berverly Hills, Malibu. The large estimated
errors may be generated in part from high prices so that a 10 percent
error in an estimate is a larger number than for lower-priced cities.
Another source of the large errors might be that the more expensive
properties sell based on features that are not in our data.
...
\code{mtry}) always improved prediction accuracy. The model in
...
why should taking away choice improve prediction when the model is
clever?
