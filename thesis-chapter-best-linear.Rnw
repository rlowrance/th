...
only 66 percent of the city names. (We say the coverage was 66 percent.)
In terms of the estimated error, the worst model was for zip codes, but
it had the highest coverage. The lowest coverage is for census tracts.
...
DO SUBMARKET MODELS OUTPERFORM INDICATOR-BASED MODELS?
...
REWORK ME AFTER INDICATOR MODELS COMPLETE.
...
Figure \ref{figure:RF30} shows the results of this experiment. Note that
...
Figure \ref{figure:RF60} shows the results of sweeping the random
