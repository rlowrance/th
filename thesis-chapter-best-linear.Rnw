...
date. An analysis showed that there were no such houses.
...
Another issue was to determine the best training period. Linear
...
model. Thus the lowest error occurred when training for 30 days and
...
To assess the extent to which regularization led to more accuracy, we
...
each submarket. The average price per feature was the same across
submarkets.
...
tried in the literature. The former appears not to have been tried.
...
generalization error was about \$15,000. A common attribute of the
...
<<RandomForests, include=FALSE>>=
error.rf <- 47035
error.L2 <- 58453
p <- 1 - (error.rf / error.L2)
p
@  
...
\$\Sexpr{Commas(error.rf)}, which was \Sexpr{Percent(p)} percent lower
than the best entire-market linear model, the one with the L2
regularizer and without the zip code indicators, which had an error of
\$\Sexpr{Commas(error.L2)}.

Thus the random forests model, with minimal design work, outperformed
the carefully designed local linear model. We traded computation time
for human time.
