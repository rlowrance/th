...
\$\Sexpr{Commas(error.lcv.15)}, which is \Sexpr{Percent(p)} lower that
the estimated error of \$\Sexpr{Commas(error.pca.3)} that was found by
PCA.
...
very small: the median is reduced about \$\Sexpr{diff} from an
...
that there is not much overfitting in the unregularized model. The
reason for that is possibly that the training sets are not very large.
In our approach, we build a local model for each query and that model
had only 30 days of training data.
