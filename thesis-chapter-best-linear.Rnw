...
compare heuristics for selecting the best features. Finally, fixing
...
separate model is build for each submarket.  We find that submarket
models outperform entire-market models.
...
fraction within 10 percent (along with fraction within 20 and 30
percent) is popular in the real estate pricing literature.
...
we set the L2 regularizer to zero and see just the rank-ordering of
...
most important features of the house is its location, but that isn't
...
jn estimated generalization error before a subsequent decline. Another
...
principal components associated with the 24 features. We see that the
...
generalization errors, but the fitting process succeeded in all folds in
...
deceptive: they are the median values across all the cities. Individual
...
Looking first at Panel A, which used the 20 predictors found to be best
for the linear models, we see that increasing \code{ntree} and
\code{mtry} always reduces the estimated error. Looking at Panel B,
which used all 24 predictors, we see that increasing \code{ntree} can
increase the error and that increasing \code{mtry} always reduced the
error. We conclude that there is some tuning to be done for the
hyperparameters.

Looking at Panel A and Panel B together, we see that the error is always
