...
Subset <- function(control) {
    ...
    str(subset.loaded)
    list( info = info
          #,df = transactions.al.sfr.subset1
          ,control = control
    )
}
...
it traded. The transactions file contains observations with unusualy values which
...
in the PRICATCODE field. The deeds files from CoreLogic contain
\Sexpr{Commas(deeds$info$nrow.all)}
deeds of which  
\Sexpr{Commas(deeds$info$nrow.arms.length)} are classified as arms-length.
...
\Sexpr{Commas(parcels$info$nrow.all)} taxroll records of which \Sexpr{Commas(parcels$info$nrow.sfr)}
...
\begin{itemize}
  \item The file of all arms-length deeds containing \Sexpr{Commas(transactions$info$nrow.deeds.al)} records.
  \item The file of all single-family-residence parcels containing \Sexpr{Commas(transactions$info$nrow.parcels.sfr)}
    records. These parcels are merged into the deeds using the best APN
    field from each file. The resulting merged file has \Sexpr{Commas(transactions$info$nrow.deeds.parcels)} records.
  \item The file containing \Sexpr{Commas(transactions$info$nrow.census)} records derived from the census tract 
    data. This file is merged into the deeds-parcels file using the
    census tract fields. A file with \Sexpr{Commas(transactions$info$nrow.census.deeds.parcels)} records results.
  \item The file containing \Sexpr{Commas(transactions$info$nrow.geocoding)} records from the geocoding provider.
    This file has the APN as its primary key and is merged into the
    census-deeds-parcel file. The resulting merged file has 
    \Sexpr{Commas(transactions$info$nrow.census.deeds.parcels.geocoding)} records.
  \item The file containing \Sexpr{Commas(transactions$info$nrow.derived.zip5)} records recording features of the
    5-digit zip codes and \Sexpr{Commas(transactions$info$nrow.derived.census.tract)} similar records from census tracts. These
    files are merged into the census-deeds-geocoding-parcels file to create the
    transactions file.
\end{itemize}


The resulting transactions file contains \Sexpr{Commas(transactions$info$nrow.transactions.al.sfr)} records.
...
Remaining <- function(used) {
    Commas(transactions$info$nrow.transactions.al.sfr - used)
}
@
...
\begin{itemize}
  ...
  \item Document type code. Transactions arising only from grant
    ...
    subset$info$DOCUMENT.TYPE.CODE$deed.of.trust)} that are neither grant nor trust deeds.
    A trust deed was used only \Sexpr{Commas(subset$info$DOCUMENT.TYPE.CODE$deed.of.trust)}
    times.
  ...
  \item Sale date. None of the deeds are missing recording dates. Some are missig
  sale dates. When a
