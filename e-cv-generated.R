...
DetermineFileNamesAndCommands <- function(control) {
    ...
    CvApplyAllPossibilities( f = file.names.commands$Accumulate
...
#    lines$Append('all: $(targets)')
    ...
    lines$Append('# this makefile generated by program e-cv-generate.R')
    lines$Append(sprintf('# e-cv-generate.R was run at %s', Sys.time()))
    lines$Append('#')
